...
#    model.add(Dense(1,activation='sigmoid'))
    ...
    x = Dense(50, name = model_sigloss.name+'_layer_1', activation='relu', kernel_regularizer=l2(0.004))(model_sigloss.inputs)
    x = Dropout(0.2)(x)
    ...
    # x = Dropout(0.2)(x)
    model_sigloss.outputs = Dense(1, name = model_sigloss.name+'_output',  activation='sigmoid', kernel_regularizer=l2(0.004))(x)
    ...
    def significanceLossMultiInvert(expected_counts,signal_mask):
        ...
        def sigLossMultiInvert(y_true,y_pred):
            ...
            result = (s+b)/(s*s+K.epsilon()) #Add the epsilon to avoid dividing by 0
            return tf.convert_to_tensor(result[0])
            # return tf.convert_to_tensor(test[0])
    ...
    # x = Dropout(0.2)(x)
    model_sigloss_multi.outputs = Dense(1, name = model_sigloss_multi.name+'_output',  activation='sigmoid')(x)
