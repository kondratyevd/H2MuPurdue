...
#    model.add(Dense(1,activation='sigmoid'))
    ...
    x = Dense(50, name = model_sigloss.name+'_layer_1', activation='relu', kernel_regularizer=l2(0.005))(model_sigloss.inputs)
    x = Dropout(0.2)(x)
    ...
    # x = Dropout(0.2)(x)
    model_sigloss.outputs = Dense(1, name = model_sigloss.name+'_output',  activation='sigmoid', kernel_regularizer=l2(0.005))(x)
    ...
    def asimovSignificanceLossInvert(expectedSignal,expectedBkgd,systematic):
        ...
        def asimovSigLossInvert(y_true,y_pred):
            ...
            log1 = tf.convert_to_tensor((s+b)*(b+sigB*sigB)/(b*b+(s+b)*sigB*sigB+K.epsilon())+K.epsilon())
            log2 = tf.convert_to_tensor(1+sigB*sigB*s/(b*(b+sigB*sigB)+K.epsilon()))

            result = 1./(2*((s+b)*K.log(log1)-b*b*K.log(log2)/(sigB*sigB+K.epsilon()))) #Add the epsilon to avoid dividing by 0
    ...
    # x = Dropout(0.2)(x)
    model_sigloss_asimov.outputs = Dense(1, name = model_sigloss_asimov.name+'_output',  activation='sigmoid')(x)
