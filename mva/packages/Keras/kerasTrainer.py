...
class KerasTrainer(object):
   ...
   #                                       period=1)
            ...
            if "sigloss" in obj.name:
                train_prediction = pandas.DataFrame(data=obj.model.predict(self.df_train_scaled[self.labels].values), columns=['pred_sig_model_sigloss'], index=self.df_train_scaled.index)
                test_prediction = pandas.DataFrame(data=obj.model.predict(self.df_test_scaled[self.labels].values), columns=['pred_sig_model_sigloss'], index=self.df_test_scaled.index)
                if self.framework.data_files:
                    data_prediction = pandas.DataFrame(data=obj.model.predict(self.data_scaled[self.labels].values), columns=['pred_sig_model_sigloss'], index=self.data_scaled.index)
            else:
